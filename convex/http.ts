...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    ...
    try {
      ...
      switch (result.type) {
        case "user.created":
          await ctx.runMutation(internal.models.users.createUser, {
          ...
          break;
        case "user.updated":
          await ctx.runMutation(internal.models.users.updateUser, {
            tokenIdentifier: `${process.env.CLERK_APP_DOMAIN}|${result.data.id}`,
            image: result.data.image_url,
          });
          break;
        case "session.created":
          await ctx.runMutation(internal.models.users.setUserOnline, {
            tokenIdentifier: `${process.env.CLERK_APP_DOMAIN}|${result.data.user_id}`,
          });
          break;
        case "session.ended":
          await ctx.runMutation(internal.models.users.setUserOffline, {
            tokenIdentifier: `${process.env.CLERK_APP_DOMAIN}|${result.data.user_id}`,
          });
          break;
      }
